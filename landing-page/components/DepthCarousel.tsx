...
const DEFAULT_ITEMS: DepthCarouselItem[] = [
  { image: '/wa_ss.png', alt: 'Slide 1' },
  { image: '/wa_ss.png', alt: 'Slide 2' },
  { image: '/wa_ss.png', alt: 'Slide 3' },
  { image: '/wa_ss.png', alt: 'Slide 4' },
  { image: '/wa_ss.png', alt: 'Slide 5' },
  { image: '/wa_ss.png', alt: 'Slide 6' }
];
...
const DepthCarousel = ({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
    >
      <div className="absolute inset-0 [transform-style:preserve-3d]" ref={rootRef}>
        {data.map((item, i) => (
          <div
            key={i}
            className="absolute left-1/2 top-1/2 cursor-pointer overflow-hidden border border-white/10 bg-[#0b0d12] shadow-[0_30px_60px_-20px_rgba(0,0,0,0.65),0_8px_20px_-10px_rgba(0,0,0,0.5)] [transform:translate(-50%,-50%)] [transform-origin:center] [will-change:transform,opacity,filter]"
            ref={el => {
      ...
      {showIndicators && count > 1 && (
        <div className="absolute bottom-4 left-1/2 z-[3000] flex -translate-x-1/2 gap-2 rounded-full bg-[rgba(14,16,22,0.4)] px-3 py-2 backdrop-blur-sm" role="tablist" aria-label="Slides">
          {data.map((_, i) => (
            <button
              key={i}
              type="button"
