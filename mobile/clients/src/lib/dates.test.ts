...
import { daysBetween, formatDateKey, lastMonthsRange, lastNDaysRange, monthRange, todayKey } from '@/lib/dates';
...
describe('todayKey / formatDateKey', () => {
  it('uses the local date late in the evening', () => {
    // 23:45 IST on the 15th.
    vi.useFakeTimers({ now: new Date('2026-09-15T18:15:00Z') });
    expect(todayKey()).toBe('2026-09-15');
    expect(formatDateKey(new Date())).toBe('2026-09-15');
  });

  it('uses the local date just after midnight, when UTC is still on the previous day', () => {
    // 00:30 IST on the 16th is 19:00 UTC on the 15th: a UTC-based key
    // (toISOString) would file today's entry under yesterday.
    vi.useFakeTimers({ now: new Date('2026-09-15T19:00:00Z') });
